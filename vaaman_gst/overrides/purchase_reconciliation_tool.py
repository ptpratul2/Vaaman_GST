...
class CustomBuildExcel(BuildExcel):
	def get_invoice_columns(self):
		...
		# Required by BuildExcel.process_data() to tag purchase-side fields
		self.pr_columns = [
		...
		columns.extend(self.pr_columns)

		return columns


def patch_build_excel():
	"""Bind CustomBuildExcel on the india_compliance module (used by download_excel_report / email)."""
